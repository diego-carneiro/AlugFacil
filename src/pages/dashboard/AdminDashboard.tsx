      ...
      <div className="space-y-8">
        <div>
          ...
          <p className="text-neutral-500 mt-1">
            Visão macro da plataforma AlugFácil.
          </p>
        </div>
        ...
        <div>
                  ...
                  {bookings.slice(0, 5).map(b => (
                    <div key={b.id} className="flex items-center gap-3 px-5 py-3">
                      <div className={`w-2 h-2 rounded-full shrink-0 ${
                        ...
                        : b.status === "confirmed" ? "bg-primary-500"
                        : b.status === "pending" ? "bg-accent-400"
                        : "bg-red-400"
                      }`} />
              ...
              <div className="bg-white rounded-2xl shadow-[0_4px_24px_rgba(0,102,204,0.08)] p-5 lg:col-span-2">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {[
                    ...
                    { label: "Ticket médio", value: `R$ ${bookings.length > 0 ? Math.round(totalRevenue / bookings.filter(b => b.status === "completed").length) : 0}`, color: "text-neutral-700" },
                  ].map(item => (
            ...
            <div className="bg-white rounded-2xl shadow-[0_4px_24px_rgba(0,102,204,0.08)] overflow-hidden">
              <div className="px-5 py-4 border-b border-neutral-100">
                <p className="font-display font-semibold text-neutral-800">
                  Todos os usuários ({users.filter(u => u.role !== "admin").length})
                </p>
              </div>
              <div className="divide-y divide-neutral-50">
          ...
          {activeTab === "bookings" && (
            <div className="bg-white rounded-2xl shadow-[0_4px_24px_rgba(0,102,204,0.08)] overflow-hidden">
              <div className="px-5 py-4 border-b border-neutral-100">
                <p className="font-display font-semibold text-neutral-800">
                  Todas as reservas ({bookings.length})
                </p>
              </div>
              <div className="divide-y divide-neutral-50">
                {bookings.map(b => (
                  <div key={b.id} className="flex items-center gap-4 px-5 py-4">
                    <img
                      src={b.consultoryImage}
                      alt={b.consultoryName}
                    ...
                    <div className="text-right shrink-0">
                      <p className="text-sm font-display font-bold text-neutral-800">
                        R$ {b.price}
                      </p>
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusColors[b.status]}`}>
                        {statusLabels[b.status]}
          ...
          {activeTab === "verifications" && (
            <div className="space-y-4">
                    ...
                    {pendingVerifications.map(user => (
                      <div key={user.id} className="flex items-center gap-4 px-5 py-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-neutral-400">{user.email}</p>
                          {user.cro && (
                            <p className="text-xs text-primary-500 mt-0.5">{user.cro}</p>
                          )}
                        </div>
              ...
              <div className="bg-white rounded-2xl shadow-[0_4px_24px_rgba(0,102,204,0.08)] overflow-hidden">
                <div className="px-5 py-4 border-b border-neutral-100">
                  <p className="font-display font-semibold text-neutral-800">
                    Usuários verificados ({users.filter(u => u.verified && u.role !== "admin").length})
                  </p>
                </div>
                <div className="divide-y divide-neutral-50">
