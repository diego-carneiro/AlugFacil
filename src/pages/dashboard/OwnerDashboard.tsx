...
import {
  useOwnerConsultories,
  useOwnerBookings,
  useConsultoryRooms,
  useInvalidateOwnerData,
} from "../../lib/queries/ownerQueries";
...
export default function OwnerDashboard() {
  ...
  const { invalidateBookings, invalidateRooms } = useInvalidateOwnerData();
  const invalidateRoomsForPrimary = () =>
    primaryConsultory ? invalidateRooms(primaryConsultory.id) : Promise.resolve();
  ...
  const {
    ...
    error: bookingsError,
  } = useOwnerBookings(ownerId);

  const {
    data: consultories = [],
    isLoading: loadingConsultories,
  } = useOwnerConsultories(ownerId);

  const primaryConsultory = consultories[0];

  const { data: rooms = [] } = useConsultoryRooms(primaryConsultory?.id);

  const isLoading = loadingBookings || loadingConsultories;
  const error =
    ...
    >
                          ...
                          <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-neutral-500 mb-2">
                            ...
                            <span className="flex items-center gap-1.5">
                              {room.periods.morning && (
                                <Sun size={13} className="text-accent-400" />
                              )}
                              {room.periods.afternoon && (
                                <Sunset size={13} className="text-accent-400" />
                              )}
                              ...
                              <div className="text-right shrink-0">
                                {b.status === "completed" ? (
                                  <>
                                    ...
                                    <p className="text-xs text-neutral-400">
                                      – R$ {Math.round(b.price * 0.1)} taxa
                                    </p>
                                  </>
                                ) : (
                                  <p
                                    className={`text-sm font-medium ${statusColors[b.status]} px-2 py-0.5 rounded-full`}
                                  >
      ...
      {reviewBooking && (
        <ReviewModal
          ...
          reviewerRole="owner"
          onSubmitted={() => ownerId && void invalidateBookings(ownerId)}
        />
      )}
      ...
      {showCreateRoom && primaryConsultory && currentUser && (
        <CreateRoomModal
          ...
          onCreated={() => {
            void invalidateRoomsForPrimary();
